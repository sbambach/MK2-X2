...
module power_base(){
	...
	translate([-3,0-8,0]) cube([3,40+8,19]);
	translate([sirka,0-8,0]) cube([3,40+8,19]);
...
module power_cuts(){
	...
	translate([sirka/2,7-8,6+tloustka_steny]) rotate([0,90,0]) cylinder(r=1.8,h=200,$fn=16,center=true);
    translate([sirka/2,9.5-8,6+tloustka_steny]) rotate([0,90,0]) cylinder(r=1.8,h=200,$fn=16,center=true);
    translate([sirka/2,7+2.5/2-8,6+tloustka_steny]) cube([200,2.5,2*1.8],center=true);
    // Zkoseni
